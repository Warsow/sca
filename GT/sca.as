...
class cCARound
{
    ...
    void newRound()
    {
        ...
        for(int i = 0; i < maxClients; i++)
        {
            ...
            if(playerStats[i].inUse)
            {
                if(client.team != TEAM_SPECTATOR)
                    playerStats[i].pRounds++;
            }
        }

        if(this.numRounds % 3 == 0 || this.numRounds == 1)
            info.displayInfo();
    }
    ...
    void newRoundState( int newState )
    {
        ...
        case CA_ROUNDSTATE_POSTROUND:
        {
            ...
            {
                G_CenterPrintMsg( null, "^1Draw Round!" );
            }
        }
        break;
        ...
        if ( this.roundStateEndTime != 0 )
        {
            ...
            if ( this.countDown > 0 )
            {
                ...
                if ( remainingSeconds < this.countDown )
                {
                    ...
                    G_CenterPrintMsg( null, String("^1" + this.countDown ) );
                }
            }
        }
        ...
        if ( this.state == CA_ROUNDSTATE_ROUND )
        {
			...
			if( this.minuteLeft > 0 )
			{
				...
				if ( caTimelimit1v1 != 0 && ( caTimelimit1v1 * 1000 ) == left )
				{
					...
					{
						G_CenterPrintMsg( null, "^1" + caTimelimit1v1 + " ^7seconds left. Hurry up!" );
					}
					else
					{
						uint minutes;					
						...
						if( seconds == 0 )
						{
							...
							if(minutes == 1) {
								G_CenterPrintMsg( null,"^1" + minutes + " ^7minute left. Hurry up!");
							} else {
								G_CenterPrintMsg( null,"^1" + minutes + " ^7minutes left. Hurry up!" );							
							}
						}
						else
						{
							minutes = ( caTimelimit1v1 - seconds ) / 60;
							G_CenterPrintMsg( null,"^1" + minutes + " minutes ^7and ^1"+ seconds +" seconds ^7left. Hurry up!"  );
						}
					}
				}
    ...
    void playerKilled( Entity @target, Entity @attacker, Entity @inflictor )
    {
        ...
        if ( @target != null && @target.client != null && @attacker != null && @attacker.client != null )
        {
			...
			{
                info.addFrag();
                playerStats[attacker.client.playerNum].pFrags++;
                playerStats[target.client.playerNum].pDeaths++;
                if(@target == @attacker)
                    playerStats[target.client.playerNum].pSuicides++;

				// report remaining health/armor of the killer
				G_PrintMsg( target, "^7You were fragged by " + attacker.client.name + "^7 (^1health: " + rint( attacker.health ) + ", armor: " + rint( attacker.client.armor ) + "^7)\n" );
			}
            ...
            {
                G_PrintMsg( null, "^11v1! ^7Good luck!\n" );
            ...
            else if ( attackerCount == 1 && targetCount > 1 )
            {
                ...
                for ( int j = 0; @team.ent( j ) != null; j++ )
                {
                    G_PrintMsg( team.ent( j ), "^11v" + targetCount + "! " + attacker.client.name + "^7 is on its own!\n" );
                }
            ...
            else if ( attackerCount > 1 && targetCount == 1 )
            {
                ...
                {
                    @ent = @team.ent( j );
                    G_PrintMsg( ent, "^11v" + attackerCount + "! " + survivor.client.name + " ^7is on its own!\n" );
                }
...
bool GT_Command( Client @client, const String &cmdString, const String &argsString, int argc )
{
    ...
    {
        if(argsString.getToken(0) == "randmap")
        {
            votes.scaRandmap();
            return true;
        }
    ...
    else if(cmdString == "callvotepassed")
    {
        if(argsString.getToken(0) == "randmap")
        {
            votes.scaRandmap(true);
            G_CmdExecute("g_maplist " + "\"" + mapRotation.nextMap + "\"");
    ...
    else if(cmdString == "!top25")
    {
        cmd.execute(client, "!top25");
        return true;
    }
    else if(cmdString == "login")
    {
        cmd.execute(client, "!login " + argsString);
        return true;
    }
    else if(cmdString == "!login")
    {
    ...
    else if(cmdString == "!logout")
    {
        cmd.execute(client, "!logout");
    }
    else if(cmdString == "!stats")
    {
        cmd.execute(client, "!stats " + argsString);
        return true;
    }
    else if(cmdString == "!authed")
    {
        cmd.execute(client, "!authed");
        return true;
    }
    else if(cmdString == "!kick")
    {
        cmd.execute(client, "!kick " + argsString);
        return true;
    }
    else if(cmdString == "!broadcast")
    {
        cmd.execute(client, "!broadcast " + argsString);
        return true;
    }
...
String @GT_ScoreboardMessage( uint maxlen )
{
    ...
    for ( t = TEAM_ALPHA; t < GS_MAX_TEAMS; t++ )
    {
        ...
        for ( i = 0; @team.ent( i ) != null; i++ )
        {
            ...
            {
                // "Name Clan Score Frags MVPs Ping R"
                entry = "&p " + playerID + " " + ent.client.clanName + " "
                        + ent.client.stats.score + " " + ent.client.stats.frags + " "
                        + base.mvpCount[ent.client.playerNum] + " " + ent.client.ping + " " + ( ent.client.isReady() ? "1" : "0" ) + " ";
            }
...
void GT_updateScore( Client @client )
{
    if ( @client != null )
    {
        ...
        else
        {
            ...
            playerStats[client.playerNum].pScore = int(client.stats.totalDamageGiven * 0.01) + caRound.getPlayerBonusScore(client);
        }
    }
}
...
void GT_ScoreEvent( Client @client, const String &score_event, const String &args )
{
    if ( score_event == "dmg" )
    {
        if ( match.getState() == MATCH_STATE_PLAYTIME )
        {
            if(@client != null)
                {
                 if(args.getToken(1).toInt() > 1000)
                 {
                    base.roundDamagePlayers[client.playerNum] += 250.0;
                    playerStats[client.playerNum].pDamageGiven += 250;
                 }
                 else
                 {
                    base.roundDamagePlayers[client.playerNum] += args.getToken(1).toInt();
                    playerStats[client.playerNum].pDamageGiven += args.getToken(1).toInt();
                 }                  
                } 
            ...
            if (@target.client != null)
            {
                if(args.getToken(1).toInt() > 1000)
                {
                    playerStats[target.playerNum].pDamageReceived += 250;
                }
                else
                {
                    playerStats[target.playerNum].pDamageReceived += args.getToken(1).toInt();
                }
            }
    ...
    {
        login.sessionCheck(client);
        base.displayTop(client, 5000);
    }
    else if ( score_event == "disconnect")
    {
        if(!playerStats[client.playerNum].inUse)
            return;

        base.mvpCount[client.playerNum] = 0;
        playerStats[client.playerNum].clearPlayer();
        alreadyLoggedOut[client.playerNum] = false;
        updateCleanPlayers(client);
        base.removeAuthedPlayer(client);
    }
    else if (score_event == "userinfochanged")
    {
        updateCleanPlayers(client, true);
   ...
   for ( int i = 0; i < maxClients; i++ )
    {
        ...
        //delayed top
        if ( top_timestamp[i] < levelTime && top_timestamp[i] != 0 )
        {
            base.displayTop(client, 0);
        }
    }
...
void GT_InitGametype()
{
    info.loadStats();
    topPlayers.getData();
    mapRotation.getMapsFromFile();
    mapRotation.writePlayedMap();
    mapRotation.getPlayedMapsFromFile();
    mapRotation.chooseNextMap();
    //G_CmdExecute("g_maplist " + "\"" + mapRotation.nextMap + "\"");
    Cvar("g_maplist", "", 1).set(mapRotation.nextMap);

    gametype.title = "Superb Clan Arena";
    gametype.version = "1.00";
    gametype.author = "Slice (& the Warsow Development Team formerly known as Vic)";
    ...
    {
        G_ConfigString( CS_SCB_PLAYERTAB_LAYOUT, "%n 112 %s 52 %i 52 %i 52 %i 40 %l 40 %r l1" );
        G_ConfigString( CS_SCB_PLAYERTAB_TITLES, "Name Clan Score Frags MVPs Ping R" );
    }

    // add votes
    G_RegisterCallvote("randmap", "", "", "Vote for a random map");
    ...
    G_RegisterCommand("!top25");
    G_RegisterCommand("!login");
    G_RegisterCommand("login");
    G_RegisterCommand("!logout");
    G_RegisterCommand("!stats");
    G_RegisterCommand("!authed");
    G_RegisterCommand("!kick");
    G_RegisterCommand("!broadcast");
